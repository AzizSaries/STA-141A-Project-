...
```{r, echo = FALSE}
get_trial_data <- function(session_data, session_id, trial_id) {
  spikes <- session_data[[session_id]]$spks[[trial_id]]
  if (any(is.na(spikes))) {
    disp("value missing")
  }

  trial_tibble <- tibble("neuron_spike" = rowSums(spikes)) %>%
    add_column("brain_area" = session_data[[session_id]]$brain_area) %>%
    group_by(brain_area) %>%
    summarize(region_sum_spike = sum(neuron_spike), region_count = n(), region_mean_spike = mean(neuron_spike)) %>%
    ungroup() %>%
    add_column("trial_id" = trial_id) %>%
    add_column("contrast_left" = session_data[[session_id]]$contrast_left[trial_id]) %>%
    add_column("contrast_right" = session_data[[session_id]]$contrast_right[trial_id]) %>%
    add_column("feedback_type" = session_data[[session_id]]$feedback_type[trial_id])
...
get_session_data <- function(session_data, session_id) {
  n_trial <- length(session_data[[session_id]]$spks)
  trial_list <- list()
  for (trial_id in 1:n_trial) {
    trial_tibble <- get_trial_data(session_data, session_id, trial_id)
    trial_list[[trial_id]] <- trial_tibble
  }
  session_tibble <- do.call(rbind, trial_list)
  session_tibble <- session_tibble %>%
    add_column("mouse_name" = session_data[[session_id]]$mouse_name) %>%
    add_column("date_exp" = session_data[[session_id]]$date_exp) %>%
...
For our first predictive model, we will use the XGBoost algorithm. We will be using this because we have a large data set. Since we were able to conclude that all 18 sessions are similar enough, we are able to pool data from all 18 sessions to create our predictive model, and XGBoost is great at handling large data sets. Additionally, we assume that there will be some multicollinearity between our predictors since, at the very least, one of our variables, the contrast difference, is a linear combination of the contrast left and contrast right variables. XGBoost is an algorithm that can handle this interaction between predictors so that is another benefit. We will be comparing this model to a generalized linear model that doesn't contain this contrast difference variable since a generalized linear model isn't suited to handle multicollinearity. We will be using AUROC as our measure of performance. AUROC is a measure that goes from 0 to 1, and the closer the number is to 1, the better your model is at predicting the binary outcome.
...
test_session_list = list()
for (session_id in 1:length(test)) {
  test_session_list[[session_id]] <- get_session_data(test, session_id)
}
